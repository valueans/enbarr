...
const Buyer = props => {
  ...
  const getInitialSerachDetail = async () => {
    ...
    if (data[1]?.length !== 0 && data.length !== 0) {
      ...
      if (data[1][0].breed_id !== null) {
        const index = getBreeds.findIndex(
          item => item.id === data[1][0].breed_id
        )
        setBreed(index === -1 ? '' : getBreeds[index])
      }

      if (data[1][0].gender !== null) {
        const genderArray = data[1][0].gender.includes(',')
          ? data[1][0].gender.split(',')
          : [`${data[1][0].gender}`]
      ...
      const colorsIndex = colors.findIndex(
        item => item.id === data[1][0].color_id
      )
      setColorList(colors)
      ...
      const deciplionIndex = deciplions.findIndex(
        item => item.id === data[1][0].discipline_id
      )
      ...
      const temperamentsIndex = temperaments.findIndex(
        item => item.id === data[1][0].temperament_id
      )
  ...
  const showResult = async () => {
    ...
    // if (!!!radius) {
    //   Alert.alert('Error', 'Please enter radius to filter results')
    //   return
    // }
    ...
    if (data[0].code == 200) {
      if (data[1].id) {
        ...
        Alert.alert('Error', 'Please Try Again with different filters.')
      }
    } else {
