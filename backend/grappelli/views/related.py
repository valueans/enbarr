    ...
    def get_filtered_queryset(self, qs):
        ...
        if query_string:
            for item in query_string.split(":"):
                k, v = item.split("=")
                if k != "_to_field":
                    filters[smart_str(k)] = prepare_lookup_value(
    ...
    def get_data(self):
        ...
        if obj_id:
            try:
                ...
                data.append(
                    {
                        "value": "%s" % self.get_return_value(obj, obj_id),
                        "label": get_label(obj),
                        "safe": get_label_safe(obj),
                    }
                )
...
class AutocompleteLookup(RelatedLookup):
        ...
        if search_fields:
            ...
            for word in term.split():
                ...
                for search_field in search_fields:
                    term_query |= models.Q(**{smart_str(search_field): smart_str(word)})
                search &= term_query
            qs = qs.filter(search)
        else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        label = ngettext("%(counter)s result", "%(counter)s results", 0) % {
            "counter": 0
        }
