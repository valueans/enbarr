...
def render_vite_bundle():
    ...
    try:
        fd = open(f"{settings.REACT_APP_DIR}/dist/manifest.json", "r")
        manifest = json.load(fd)
    except:
        raise Exception(
            f"Vite manifest file not found or invalid. Maybe your {settings.REACT_APP_DIR}/dist/manifest.json file is empty?"
        )

    imports_files = "".join(
        [
            f'<script type="module" src="/static/{manifest[file]["file"]}"></script>'
            for file in manifest["src/dev/previews.jsx"]["imports"]
        ]
    )
